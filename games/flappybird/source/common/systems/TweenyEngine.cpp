...
void TweenyEngine::add(int original, std::function<void(int)> setter, int target, int durationInMilliseconds, bool bounce)
{
    ...
    if (bounce)
    {
        tween = tweeny::from(original).to(target).during(durationInMilliseconds).via(easing::elasticOut);
    }
    else
    {
        tween = tweeny::from(original).to(target).during(durationInMilliseconds).via(easing::linear);
    }
