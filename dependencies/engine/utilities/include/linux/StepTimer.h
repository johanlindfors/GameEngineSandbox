...
namespace Utilities
{
	...
	public:
		...
		void Tick(std::function<void()> update) {
			...
			uint64_t timeDelta = std::chrono::duration_cast<std::chrono::microseconds>(currentTime - m_qpcLastTime).count();
			std::cout << "Time difference:" << timeDelta << std::endl;
			...
			{
				std::cout << "Is fixed time step!" << std::endl;
	...
	private:
		...
		Clock::time_point m_qpcLastTime;
