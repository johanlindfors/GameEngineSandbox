...
namespace Engine
{
	...
	public:
		...
		template<class T>
		std::shared_ptr<T> loadModel(const std::string &fileName) {
			...
			if (file && file->isOpen())
			{
				...
				while (!feof(fileHandle))
				{
					...
					else if(instruction == "f")
					{
						...
						if constexpr (std::is_same<T, Model<VertexPositionTexture>>()) {
							int v[MAX_FACES], u[MAX_FACES];
							int i = 0;
							while(i < MAX_FACES && fscanf(fileHandle, "%i/%i ", &v[i], &u[i]) > 0)
								i++;
							if(i >= 3) {
								vertices.emplace_back(VertexPositionTexture(verts[v[0]-1], uvs[u[0]-1]));
								vertices.emplace_back(VertexPositionTexture(verts[v[1]-1], uvs[u[1]-1]));
								vertices.emplace_back(VertexPositionTexture(verts[v[2]-1], uvs[u[2]-1]));
							}
							if(i == 4) {
								vertices.emplace_back(VertexPositionTexture(verts[v[0]-1], uvs[u[0]-1]));
								vertices.emplace_back(VertexPositionTexture(verts[v[2]-1], uvs[u[2]-1]));
								vertices.emplace_back(VertexPositionTexture(verts[v[3]-1], uvs[u[3]-1]));
							}
						} else if constexpr (std::is_same<T, Model<VertexPositionNormalTexture>>()) {
							int v[MAX_FACES], n[MAX_FACES], u[MAX_FACES];
							int i = 0;
							while(i < MAX_FACES && fscanf(fileHandle, "%i/%i/%i ", &v[i], &u[i], &n[i]) > 0)
								i++;
							if(i >= 3) {
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[0]-1], normals[n[0]-1], uvs[u[0]-1]));
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[1]-1], normals[n[1]-1], uvs[u[1]-1]));
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[2]-1], normals[n[2]-1], uvs[u[2]-1]));
							}
							if(i == 4) {
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[0]-1], normals[n[0]-1], uvs[u[0]-1]));
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[2]-1], normals[n[2]-1], uvs[u[2]-1]));
								vertices.emplace_back(VertexPositionNormalTexture(verts[v[3]-1], normals[n[3]-1], uvs[u[3]-1]));
							}
						}
					}
